...
/* ═══════════════════════════════════════════════════════════
   DEAL PULSE — Sales Agent Portal
   Each agent sees ONLY their own deals (Aug 2024 → today)
   with milestone tracking, progress, and drawer details.
   ═══════════════════════════════════════════════════════════ */
...
const COLUMNS: ColDef[] = [
  ...
  { label: "Appt Setter", key: "appointment_setter", type: "text", hideInTable: true },
  { label: "CC Setter", key: "call_center_appointment_setter", type: "text", hideInTable: true },
];
...
export default function DealPulse() {
  useEffect(() => {
  ...
  /* ─── Load deals (scoped to agent) ─── */
  const load = useCallback(async () => {
    if (!agentName) { setRows([]); setLoading(false); return; }
    setLoading(true); setMsg(null);

    let q = supabase
      .from("deals_view")
      .select(SELECT_FIELDS)
      .eq("sales_rep", agentName)
    ...
    setLoading(false);
  }, [agentName, startDate, endDate, customerQ, statuses]);
  ...
  /* ─── Options from data ─── */
  const statusOptions = useMemo(() => uniqSorted(rows.map((r) => r.status)), [rows]);
  ...
  function clearAll() { setStatuses([]); setCustomerQ(""); setStartDate(DEFAULT_START); setEndDate(""); }
  ...
  if (!agentLoading && !agentName) {
    return (
      <div className="min-h-0 flex flex-col items-center justify-center bg-slate-50 p-8" style={containerStyle}>
        <div className="text-center">
          ...
          <h3 className="text-lg font-semibold text-slate-800 mb-1">No Agent Profile Linked</h3>
          <p className="text-sm text-slate-500 max-w-sm">
            Your account is not linked to a sales rep name. Please contact your admin to set up your <span className="font-medium">linked_name</span> in User Management.
          </p>
        </div>
      </div>
    );
          ...
          <div className="p-4 space-y-3">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-base font-bold text-slate-800 tracking-tight">DealPulse</h2>
                  <p className="text-xs text-slate-400">
                    Your active deals · <span className="font-medium text-slate-500">{agentName}</span>
                  </p>
                </div>
              </div>
              <div className="flex gap-2 items-center">
                <span className="inline-flex items-center rounded-md px-2 py-0.5 text-[11px] font-semibold border border-blue-200 bg-blue-50 text-blue-700">
                  Agent View
                </span>
            ...
            <div className={`${UI.card} p-3`}>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                ...
                <div>
                  ...
                  <MultiSelect options={statusOptions} selected={statuses} onChange={setStatuses} placeholder="All Active" />
                </div>
                <div>
        ...
        <div className="flex-shrink-0 flex items-center justify-between px-5 py-4 border-b border-slate-200">
          <div className="min-w-0">
            ...
            <div className="text-[10px] text-slate-400 mt-0.5">
              {deal.company ?? "No company"} · Closed {fmtDate(deal.date_closed)}
            </div>
          </div>
